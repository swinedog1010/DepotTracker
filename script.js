...
(() => {
    ...
    function openModal() {
        ...
        // Kurz warten, damit der Browser die Animation rendern kann.
    ...
    emailForm.addEventListener("submit", async (event) => {
        ...
        try {
            // POST an Backend (PHP). JSON-Body wie im Prompt vorgegeben.
            const res = await fetch("save_email.php", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ email: value }),
            });

            // Wir parsen JSON nur, wenn die Antwort tatsaechlich JSON ist.
            ...
            closeModal();
            showToast("E-Mail erfolgreich gespeichert.", "success");
        } catch (err) {
            // Wenn der Server nicht erreichbar ist (z.B. statisches Hosting),
            // speichern wir die E-Mail wenigstens lokal und blockieren das
            // UI nicht. So bleibt das Dashboard auch ohne Backend nutzbar.
            console.warn("save_email.php nicht erreichbar:", err);
            localStorage.setItem(STORAGE_KEY, value);
            closeModal();
            showToast("E-Mail lokal gespeichert (Server offline).", "error");
        } finally {
            emailSubmit.disabled = false;
            emailSubmit.textContent = "Speichern & fortfahren";
        }
    });
